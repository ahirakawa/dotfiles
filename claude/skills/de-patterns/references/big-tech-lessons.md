...
## Airbnb: COVID が予測モデルを壊した教訓と構造的変化への対応
...
### アーキテクチャ判断: volume と timing の分離

- 予約日軸の総量（gross metrics）とリードタイム構成（lead-time composition）を別々にモデリングする設計判断がCOVID対応で最も効果が高かった
- 従来は予約→旅行を一体で予測していたが、ロックダウン・再開・再ロックダウンのサイクルで「量の変動」と「構造の変化」が絡み合い、一体モデルでは診断不能に
- **教訓**: 時間シフトのあるメトリクス（予約と実行が時間差で発生するもの）では、volume と timing の分離を検討すべき。保険金請求、サブスク更新、サプライチェーン発注にも応用可能

### B-DARMA モデル（Bayesian Dirichlet ARMA）

- Dirichlet分布でリードタイム構成の simplex 制約（非負・合計1）を自然に扱う
- log-ratio 空間でのベクトル自己回帰プロセスで時間的依存関係を学習
- ベイズフレームワークにより事前知識の注入と不確実性の定量化が可能
- 派生モデル: B-DARCH（時変ボラティリティ対応）、高次元用の shrinkage prior

### 構造的変化の検知: ダミー変数では不十分

- COVIDをダミー変数で対処 → in-sample は良いが out-of-sample で失敗。各ショックに個別パラメータが必要になり汎化不能
- **Directional-Shift 介入メカニズム**: 方向ベクトル（どの構成要素が増減するか）+ 振幅パラメータ + ロジスティックゲート関数（遷移の速度とタイミング）の3要素に分解
- ロジスティックゲートは急激なショック（急カーブ）も緩やかな遷移（緩カーブ）も表現でき、未観測期間への外挿が可能
- **結果**: Aitchison距離 31%改善、80%予測区間のカバレッジが54%→80%に回復

### 「分布は元に戻らなかった」問題

- 2021年後半に予約総量は回復したが、リードタイム分布はCOVID前に戻らず新しい形に定着
- 正規化L1ダイバージェンスでベースラインからの分布的乖離を定量監視
- 国際旅行者は国内旅行者より乖離が大きく回復も遅い
- **教訓**: ポイント統計（平均・中央値）ではなく分布全体を監視すべき。分布のドリフトはトップラインの誤差指標に現れる前に検知できる

---

## Pinterest: 社内 MCP エコシステムの構築と運用

Sources:
- https://medium.com/pinterest-engineering/building-an-mcp-ecosystem-at-pinterest-d881eb4c16f1

### アーキテクチャ: ドメイン別 MCP サーバー + 中央レジストリ

- 1つの巨大MCPサーバーではなく、ドメインごとに小さなMCPサーバーを分離する設計を選択
- サーバーごとにアクセス制御を分離でき、モデルのコンテキストを圧迫しない
- 中央レジストリでサーバーを管理し、IDE・社内チャット・AIエージェントから統一的にアクセス

### 主要 MCP サーバー

- **Presto MCP**: 最もトラフィックが多い。エージェントがダッシュボードに切り替えずにデータを直接取得
- **Spark MCP**: Spark ジョブの障害診断、ログ要約、構造化された根本原因分析の記録を支援
- **Knowledge MCP**: 社内ドキュメント・デバッグ情報への汎用ナレッジエンドポイント

### 規模と成果

- 月間 66,000回以上の呼び出し
- 推定 月7,000時間の工数削減
- **教訓**: 高レバレッジな少数のMCPサーバーから始め、他チームがその上に構築できるようにすることでエコシステムが有機的に成長した
